...
class OptimizationAnalysis:
    ...
    def _load_data(self, timestamps=None):
        ...
        for timestamp in timestamps:
            ...
            with h5py.File(self.filename, "r") as f:
                ...
                self.data[timestamp]["gammas"] = f[timestamp]["gammas"][()]
                self.data[timestamp]["alphas1"] = f[timestamp]["alphas1"][()]
                self.data[timestamp]["alphas2"] = f[timestamp]["alphas2"][()]
    ...
    def best_circuit(self, timestamp=None):
        ...
        gammas = self.get_data(timestamp)["gammas"][-1][idx]
        alphas1 = self.get_data(timestamp)["alphas1"][-1][idx]
        alphas2 = self.get_data(timestamp)["alphas2"][-1][idx]
    ...
    def best_U_tot(self, timestamp=None):
        ...
        opt.set_tf_vars(betas,gammas, alphas1, alphas2, phis, thetas)
...
#             if not discrete
#             else plt.get_cmap(
#                 "plasma_r", int(np.max(parameters) - np.min(parameters) + 1)
#             )
#         )
...
#         ax.set_xlabel(sweep_param_name, size=8)
#         ax.set_ylabel(
#             "Minimum N Blocks to\nreach " + str(100 * success_fid) + "% Fidelity",
#             size=8,
#         )
#         if fit is not None:
