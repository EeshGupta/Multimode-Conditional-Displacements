...
#                 )
    ...
    if curvature_correction:
        ...
        print('---------------------------------------------------')
        print('final displacement is '+ str(current_beta))
...
def conditional_displacement_circuit(
    ...
    #EG: what does this mean? **have not edited this
    if double_CD:
    ...
    for i, beta in enumerate(betas):
        ...
        #     dr, di = storage1.displace.make_wave(pad=False)
        #     e_cd1 = (
        #         np.abs(beta / 2.0)
        #         * storage1.displace.unit_amp
        #         * (dr + 1j * di)
        #         * np.exp(1j * np.angle(beta))
        #     )
        #     o_cd1 = np.zeros_like(e_cd1)
        #     ap = np.zeros_like(
        #         e_cd1
...
#         else:
        if (
            ...
            e_cd1, o_cd1, alpha1, tw1 = conditional_displacement(
                beta,
                alpha=alpha_CD1,
                storage=storage1,
                qubit=qubit,
        ...
        analytic_dict = analytic_CD(
            -1j * 2 * np.pi * 1e-3 * storage1.epsilon_m_MHz * e_cd1,
            o_cd1,
            2 * np.pi * 1e-6 * storage1.chi_kHz,
        )
        ...
        #---------------------------------------------
        # Constructing second cavity part
        #---------------------------------------------
        gamma = gammas[i]
...
#         if (
#             i == len(gammas) - 1 and final_disp
#         ):  # todo: could put this final displacement at the g frequency...
#             dr, di = storage2.displace.make_wave(pad=False)
#             e_cd2 = (
#                 np.abs(gamma / 2.0)
#                 * storage2.displace.unit_amp
#                 * (dr + 1j * di)
#                 * np.exp(1j * np.angle(gamma))
#             )
#             o_cd2 = np.zeros_like(e_cd2)
#             ap = np.zeros_like(
#                 e_cd2
#             )  # todo: update this... Can accumulate phase on this displacement also...
#         if gamma == -1 * last_gamma:
#             e_cd2 = -1 * e_cd2
        if (
            True#gamma != last_gamma
        ):  # don't construct the next one if it's the same beta...no need...
            e_cd2, o_cd2, alpha2, tw2 = conditional_displacement(
                gamma,
                alpha=alpha_CD2,
                storage=storage2,
                qubit=qubit,
        ...
        # getting the phase for the phase correction
        analytic_dict2 = analytic_CD(
            -1j * 2 * np.pi * 1e-3 * storage2.epsilon_m_MHz * e_cd2,
            o_cd2,
            2 * np.pi * 1e-6 * storage2.chi_kHz,
        )
        cd_qubit_phases.append(analytic_dict2["qubit_phase"])
        analytic_gammas.append(analytic_dict2["gamma"])

#         ###gamma less than 10**-3
#         else:
#             e_cd2, o_cd2 = np.array([]), np.array([])
#             cd_qubit_phases.append(0)
        
        last_gamma = gamma
        #----------------------------------------------
        # constructing qubit part (still inside beta for loop)
        #----------------------------------------------
        pr, pi = qubit.pulse.make_wave(pad=False)
        # doing the same thing the FPGA does
        detune = qubit.pulse.detune
        ...
        for j in range(len(thetas)):
            ...
            qubit_dac_pulse[j].append(np.exp(-1j * cumulative_qubit_phase) * o_cd1)
            qubit_dac_pulse[j].append(np.exp(-1j * cumulative_qubit_phase) * o_cd2)
            if buffer_time > 0 and len(qubit_dac_pulse[0]) > 0:
                qubit_dac_pulse[j].append(np.zeros(buffer_time))

        # constructing cavity  part
        cavity1_dac_pulse.append(np.zeros(len(o_r)))
        cavity2_dac_pulse.append(np.zeros(len(o_r)))
        if buffer_time > 0 and len(qubit_dac_pulse[0]) > 0:  #buffer before cavity pulse NOT for qubit echo
            cavity1_dac_pulse.append(np.zeros(buffer_time))
            cavity2_dac_pulse.append(np.zeros(buffer_time))
            
        cavity1_dac_pulse.append(beta_sign * e_cd1)    # cavity1 pulse
        cavity2_dac_pulse.append(np.zeros(len(e_cd1)))
        
        cavity2_dac_pulse.append(gamma_sign * e_cd2)    # cavity2 pulse
        cavity1_dac_pulse.append(np.zeros(len(e_cd2)))
        
        
        if echo_qubit_pulses:                       #buffer after cavity pulse,NOT for qubit echo
            beta_sign = beta_sign * -1
            gamma_sign = beta_sign * -1
        if buffer_time > 0 and len(qubit_dac_pulse[0]) > 0:
            cavity1_dac_pulse.append(np.zeros(buffer_time))
            cavity2_dac_pulse.append(np.zeros(buffer_time))

        # update the frame of the qubit
        cumulative_qubit_phase += cd_qubit_phases[-1] + cd_qubit_phases[-2]

    ##########################Finish betas for loop
    cavity1_dac_pulse = np.concatenate(cavity1_dac_pulse)
    cavity2_dac_pulse = np.concatenate(cavity2_dac_pulse)
    ...
    accumulated_phase1 = np.zeros_like(cavity1_dac_pulse)
    accumulated_phase2 = np.zeros_like(cavity2_dac_pulse)

#     if pad:
#         while (len(cavity1_dac_pulse) % 4 != 0 and len(cavity1_dac_pulse) < 24 and 
#         (len(cavity2_dac_pulse) % 4 != 0 and len(cavity2_dac_pulse) < 24):
#             cavity1_dac_pulse = np.pad(cavity1_dac_pulse, (0, 1), mode="constant")
#             cavity2_dac_pulse = np.pad(cavity2_dac_pulse, (0, 1), mode="constant")
#             qubit_dac_pulse = [
...
#             ]
    ...
    return_dict = {
        "cavity1_dac_pulse": cavity1_dac_pulse,
        "cavity2_dac_pulse": cavity2_dac_pulse,
        "qubit_dac_pulse": qubit_dac_pulse,
        "accumulated_phase1": accumulated_phase1,
        "accumulated_phase2": accumulated_phase2,
        "flip_idxs": flip_idxs,
        "alphas1": alphas1,
        "alphas2": alphas2,
        "tws1": tws1,
        ...
        "analytic_gammas": analytic_gammas
    }
    return return_dict
