...
#                 )
    ...
    def insert_user_initial_param(self, current_angles, user_angles_specfic):
        ...
        #checking if user specified the multistatr
        np_user_ang = np.array(user_angles_specfic)
        # print('before')
        # print(user_angles_specfic)
        if len(np_user_ang.shape) == 1 or len(np_user_ang.shape) == 0: 
            #reshape array assuming num blocks is correct!
            np_user_ang = np_user_ang.reshape(N_blocks, 1)
            user_angles_specfic = np_user_ang
        # print('after')
        # print(user_angles_specfic)
        N_provided_ms = len(user_angles_specfic[0])
      
        #N_actual_ms = self.parameters["N_multistart"]
    ...
    def randomize_and_set_vars(self):
        ...
        if self.user_angles is not None:
            ...
            phis = self.insert_user_initial_param(phis, self.user_angles[2])
            thetas = self.insert_user_initial_param(thetas, self.user_angles[3])
    ...
    def best_fidelity(self):
        ...
        if self.parameters['N_multistart'] == 1: 
            return fids.numpy()
        #otherwise do this
    ...
    def save_angles(self, filename):
        ...
        # if type(betas) is not np.ndarray: 
